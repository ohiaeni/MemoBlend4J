...
2. ブランチの作成を行う。まず、現在のブランチを確認する。
    
    下図では、mainブランチが表示されている。コードの修正を行う場合は、mainブランチでは修正を行わずに、別のブランチを作成して修正を行う。

    <div>
    <img src="./images/check-branch.png" alt="ブランチの確認">
    </div>
    ...
    >[!INFO]
    >もし、ブランチを削除したい場合は、画像左の「ソース管理」をクリックする。その後、画像の3点リーダーをクリックしてメニューを表示する。「ブランチ」、「ブランチの削除」を順に選択する。
    ><div>
    ><img src="./images/delete-branch-example.png" alt="ブランチを削除する例">
    ></div>
    >すると、削除するブランチを選択するメニューを選択する画面が表示されるので、適切に選択し削除を実行する。
    ><div>
    ><img src="./images/select-delete-branch.png" alt="削除するブランチを選択する例">
    ></div>
...
ここまでで、ローカルリポジトリの変更を行うことができる。しかし、リモートリポジトリには、変更をまだ伝えていない。以降の手順でローカルリポジトリの変更内容を、リモートリポジトリの管理人に伝える。これを"プッシュ"と呼ぶ。「コミット」という文言が「変更の同期」または「ブランチの発行」に変化するので、そのボタンをクリックしてプッシュする。すると、リモートリポジトリにも"feature/gitの練習"ブランチが作成され、ローカルリポジトリの"feature/gitの練習"ブランチの変更内容が伝えられる。
...
3. ローカルリポジトリのmainブランチから、"feature/<任意の名前>"ブランチを作成する。
    ...
    <div>
    <img src="./images/create-branch.png" alt="ブランチを切り替える方法">
    </div>
